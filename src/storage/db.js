 ...
 * Includes JSON file fallback to ensure compatibility in any environment.
 */
...
try {
  ...
  if (!fs.existsSync(JSON_FALLBACK_PATH)) {
    fs.writeFileSync(
      JSON_FALLBACK_PATH,
      JSON.stringify({ batches: {}, logs: [], ai_explanations: {} }, null, 2)
    );
  }
}
...
function getJsonData() {
  try {
    ...
    return { batches: {}, logs: [], ai_explanations: {} };
  }
}
